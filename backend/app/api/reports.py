...
from app.core.dependencies import require_inspector_or_admin
...
def _build_report_payload(scan: ScanJob) -> dict:
    ...
    if (
        "usp_cross_verification" not in master_report
        and "usp_cross_verification" in extracted
    ):
        master_report["usp_cross_verification"] = extracted[
            "usp_cross_verification"
        ]

    if (
        "second_schedule_shrinkflation_audit" not in master_report
        and "second_schedule_shrinkflation_audit" in extracted
    ):
        master_report["second_schedule_shrinkflation_audit"] = extracted[
            "second_schedule_shrinkflation_audit"
        ]

    if (
        "visual_and_metrology_audit" not in master_report
        and "visual_and_metrology_audit" in extracted
    ):
        master_report["visual_and_metrology_audit"] = extracted[
            "visual_and_metrology_audit"
        ]

    # Filter statutory_summary so compliance affirmations
    # are never treated as violations.
    raw_summary = (
        master_report.get("statutory_summary")
        or extracted.get("statutory_summary")
        or []
    )
    ...
    clean_summary = [
        ...
        if "comply strictly" not in item.lower()
        and "no statutory" not in item.lower()
        and "nil" not in item.lower()
    ]
    ...
    image_urls = list(scan.image_urls or [])

    if scan.image_url and scan.image_url not in image_urls:
        image_urls.append(scan.image_url)

    # Evidence image attached by inspector
    evidence_image_urls = []

    if getattr(scan, "back_image_url", None):
        evidence_image_urls.append(scan.back_image_url)
    ...
    return {
        ...
        "inspector_name": (
            scan.inspector.full_name
            if scan.inspector
            else "Senior Inspector"
        ),
        ...
        "cross_referenced_fields": (
            scan.cross_referenced_fields
            or extracted.get("cross_referenced_fields", [])
        ),
...
def _generate_and_store_reports(scan: ScanJob, db: Session) -> dict:
...
def generate_reports(
    ...
    result = _generate_and_store_reports(scan, db)

    return {
        **result,
        "message": (
            "PDF and DOCX compliance reports "
...
def download_pdf_report(
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": (
                f"attachment; "
                f"filename=VisionX_Report_{scan_id[:8]}.pdf"
            )
        },
    )
...
def download_docx_report(
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": (
                f"attachment; "
                f"filename=VisionX_Report_{scan_id[:8]}.docx"
            )
        },
    )
...
async def attach_evidence_photo(
    ...
    if not content_type.startswith("image/"):
        raise HTTPException(
            status_code=400,
            detail="Only image evidence files are allowed",
        )
    ...
    evidence_filename = (
        f"evidence_{scan_id}_{original_name}"
    )
    ...
    additional_note = (
        f"\n[Evidence Attached: "
        f"{original_name} - Notes: {notes}]"
    )

    scan.inspector_notes = (
        existing_notes + additional_note
    )

    # Store the evidence URL in the scan record.
    ...
    # IMPORTANT:
    # Regenerate both reports immediately after evidence
    # is attached so the evidence is included in the
    # generated report.
    report_result = _generate_and_store_reports(
        scan,
        db,
    )
